<h3>All Sales Data</h3>
@if ($count == 0)
    <p class="text-secondary">The database is empty.</p>
@endif
@php
    // Find the Transaksi with the most Transaksi_Barang
    $maxTransaksi = $count ? $general->sortByDesc(fn($t) => $t->transaksi_barang->count())->first() : null;

    // Get the maximum count of Transaksi_Barang
    $maxCount = $maxTransaksi ? $maxTransaksi->transaksi_barang->count() : 0;
@endphp
<div class="table-responsive mb-4">
    <table class="table table-hover" style="width: {{$maxCount == 0 ? 'initial' : (90 * $maxCount).'vw'}}">
